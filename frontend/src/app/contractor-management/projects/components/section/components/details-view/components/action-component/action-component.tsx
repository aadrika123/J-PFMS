...
import { useContractApprovalCommentList } from "./comment-data";
...
export const ActionComponent: React.FC<ActionComponentPropsType> = (props) => {
  ...
  const { data: commentList } = useContractApprovalCommentList(props.proposalId);
  ...
  return (
    <>
      ...
      <div className="flex mt-4 justify-between">
        ...
        <div>
          Comments:
          {commentList && commentList.length >0 && commentList?.map((item: any, index: number) => {
            return (
              <>
                <hr className="mb-4" />
