...
export const tenderWorkDetailsSchema = Yup.object({
  ...
  completion_period: Yup.number()
    .required("completion period is required").min(1, 'less then 1 is not allowed')
  ...
  pin_code: Yup.number()
    .required("pin code is required").min(1, 'less then 1 is not allowed')
  ...
  bid_opening_place: Yup.string().required("bid opening place is required"),
  pre_bid_meeting_place: Yup.string().required(
    "pre bid meeting place is required"
  ),
  pre_bid_meeting_address: Yup.string().required(
    "pre bid meeting address is required"
  ),
